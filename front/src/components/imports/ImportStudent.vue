...
const handleDelete = async () => {
  const studentsToDelete = await studentStore.fetchStudents(
    props.year,
  ...
  if (fileInput.files && fileInput.files.length > 0) {
    ...
    reader.onload = async (e) => {
      ...
      if (data) {
        ...
        const rows: (string | undefined)[][] = XLSX.utils.sheet_to_json(
          worksheet,
          { header: 1 },
        );
        ...
        successMessage.value = "Les étudiants ont été importés avec succès.";
      }
    };
